    ...
    books.search(bookToSearch, options, function (err, res) {
      ...
      for (var i = 0; i < res.length; i++) {
        ...
        if (res[i].hasOwnProperty('description')) {
          ...
          var shortDescr = '';
          if (res[i].description.length > maxDescr) shortDescr = res[i].description.substr(0, maxDescr) + '...';else shortDescr = res[i].description;
...
function show(req, res) {
  return getBooks(req.params.id).then(respondWithResult(res)).catch(handleError(res));
}
...
//# sourceMappingURL=book-search.controller.js.map
